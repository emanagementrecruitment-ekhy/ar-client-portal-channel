...
export default function AdminClientLedgerPage() {
  ...
  return (
    <div>
      ...
      <div className="pt-5.5 grid grid-cols-1 lg:[grid-template-columns:1fr_1.6fr] gap-4">
        ...
        <div className="p-5 bg-[var(--surface)] border border-[var(--line)] rounded-2xl h-fit overflow-x-auto">
          <div className="text-[16px] font-semibold text-[var(--gold2)] mb-3.5">Ledger {monthLabel(month)}</div>
          <table className="w-full text-[11px] border-collapse min-w-[720px]">
            <thead>
              <tr className="text-left text-[var(--dim)] uppercase text-[9.5px] tracking-[0.1em] border-b border-[var(--line)]">
                <th className="py-2 pr-2">No</th>
                <th className="py-2 pr-2">Nama</th>
                <th className="py-2 pr-2">Outlet</th>
                <th className="py-2 pr-2 text-right">VCR</th>
                <th className="py-2 pr-2 text-right">Fee</th>
                <th className="py-2 pr-2 text-right">Jumlah</th>
                <th className="py-2 pr-2 text-right">Potongan/Kasbon</th>
            ...
            <tbody>
              {entries.length === 0 && (
                <tr>
                  <td colSpan={9} className="py-4 text-[var(--faint)]">
              ...
              {entries.map((e, i) => (
                <tr key={e.id} className="border-b border-[var(--line)]/60">
                  <td className="py-2 pr-2 text-[var(--dim)]">{i + 1}</td>
                  <td className="py-2 pr-2">{e.nama}</td>
                  <td className="py-2 pr-2 text-[var(--dim)]">{e.outlet}</td>
                  <td className="py-2 pr-2 text-right">{e.vcr}</td>
                  <td className="py-2 pr-2 text-right">{fmtRp(e.fee)}</td>
                  <td className="py-2 pr-2 text-right text-[var(--gold2)]">{fmtRp(e.jumlah)}</td>
                  <td className="py-2 pr-2 text-right text-[var(--red)]">{e.potongan ? fmtRp(e.potongan) : "—"}</td>
                  <td className="py-2 pr-2 text-[var(--dim)]">{e.keterangan || "—"}</td>
                  <td className="py-2 pr-2 whitespace-nowrap">
                    <button onClick={() => startEdit(e)} className="text-[var(--gold)] text-[10.5px] mr-2.5 cursor-pointer">
                      Edit
                    </button>
                    <button onClick={() => remove(e.id)} className="text-[var(--red)] text-[10.5px] cursor-pointer">
                      Hapus
                    </button>
                  </td>
                </tr>
            ...
            {entries.length > 0 && (
              <tfoot>
                <tr className="border-t border-[var(--goldline)] font-semibold">
                  ...
                  <td className="py-2.5 pr-2 text-right">{totals.vcr}</td>
                  <td className="py-2.5 pr-2"></td>
                  <td className="py-2.5 pr-2 text-right text-[var(--gold2)]">{fmtRp(totals.jumlah)}</td>
                  <td className="py-2.5 pr-2 text-right text-[var(--red)]">{fmtRp(totals.potongan)}</td>
                  <td colSpan={2}></td>
                </tr>
                <tr>
                  <td className="py-2.5 pr-2 text-[var(--gold)] uppercase text-[10px] tracking-[0.14em]" colSpan={5}>
                    Grand Total
                  </td>
                  <td className="py-2.5 pr-2 text-right text-[var(--gold2)] font-semibold text-[16px]" colSpan={4}>
